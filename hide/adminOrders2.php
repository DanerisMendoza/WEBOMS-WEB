      ...
      ?>
                <form method="get">
                    <select name="sort" class="form-control form-control-lg col-12 mb-3" method="get">
                        <?php 
                if(isset($_GET['sort'])){
                  ?><option value="<?php echo $_GET['sort'];?>" selected>Sort: <?php echo strtoupper($_GET['sort']);?>
                        </option><?php
                }else{
                ...
                <table class="table table-striped table-bordered col-lg-12">
                  ...
                  ?>
                            ...
                            <td><a class="btn btn-light border-dark"
                                    href="adminOrder_details.php?idAndPic=<?php echo $row['order_id']?>">Order
                                    Details</a></td>

                            <!-- option -->
                            <?php 
                    if($row['status'] == 'prepairing'){
                      ?> <td colspan='2'><a class="btn btn-success border-dark"
                                    href="?serve=<?php echo $row['order_id'] ?>">Serve</a></td><?php
                    }
                    elseif($row['status'] == 'serving'){
                      ?> <td colspan="2"><a class="btn btn-success border-dark"
                                    href="?orderComplete=<?php echo $row['order_id'] ?>">Order Complete</a></td><?php
                    }
                    elseif($row['status'] == 'complete'){
                  ...
                  ?>
                            ...
                            <td><a class="btn btn-danger border-dark"
                                    href="?delete=<?php echo $row['ID'].','.$row['order_id'] ?>">Delete</a></td>
                        </tr><?php } ?>
                    </tbody>
                </table>
                <?php } ?>
